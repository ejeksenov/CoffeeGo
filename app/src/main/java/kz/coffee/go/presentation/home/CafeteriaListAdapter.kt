...
class CafeteriaListAdapter: RecyclerView.Adapter<CafeteriaListAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding: RowCafeteriaListItemLayoutBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.row_cafeteria_list_item_layout, parent, false)
    ...
    inner class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        private val tvCafeteriaStartingPrice: TextView = itemView.findViewById(R.id.tvCafeteriaStartingPrice)

        fun bind(cafeteria: Cafeteria) {
            Glide.with(itemView.context).load(cafeteria.imageUrl).placeholder(R.drawable.ic_image_gray_200dp).error(R.drawable.ic_broken_image_gray).centerCrop().into(ivCafeteriaImage)
            Glide.with(itemView.context).load(cafeteria.logoUrl).placeholder(R.drawable.ic_image_gray_34dp).error(R.drawable.ic_broken_image_gray).centerCrop().into(ivCafeteriaLogo)
